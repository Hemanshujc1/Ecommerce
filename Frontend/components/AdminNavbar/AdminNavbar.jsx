...
const AdminNavbar = ({ onClose, isCollapsed, toggleCollapse }) => {
  ...
  const baseNavLinks = [
    ...
    {
      href: "/admin/EditLandingPage",
      label: "Edit Landing Page",
      icon: <FaEdit />,
    },
    {
      href: "/admin/HomePageControl",
  ...
  return (
    <div className="h-full w-full bg-[#2a2a2a] text-white flex flex-col shadow-lg relative transition-all duration-300">
      {/* Header */}
      <div
        className={`p-4 border-b border-gray-700 flex items-center flex-shrink-0 relative ${isCollapsed ? "justify-center" : "justify-between"}`}
      >
        {!isCollapsed ? (
          ...
          <div className="font-bold text-xl h-10 flex items-center justify-center">
            A
          </div>
        )}
      ...
      {/* Nav links */}
      <nav
        className={`flex-1 overflow-y-auto p-3 flex flex-col gap-1 overflow-x-hidden ${isCollapsed ? "items-center" : ""}`}
      >
